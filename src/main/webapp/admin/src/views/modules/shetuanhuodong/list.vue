<template>
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
        ...
        <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
          <el-form-item>
            <el-button
              v-if="isAuth('shetuanhuodong','新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1"
              type="success"
              icon="el-icon-plus"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'新增':'' }}</el-button>
            <el-button
              v-if="isAuth('shetuanhuodong','新增') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2"
              type="success"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'新增':'' }}<i class="el-icon-plus el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('shetuanhuodong','新增') && contents.btnAdAllIcon == 0"
              type="success"
              @click="addOrUpdateHandler()"
            >{{ contents.btnAdAllFont == 1?'新增':'' }}</el-button>
            <el-button
              v-if="isAuth('shetuanhuodong','删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 1 && contents.tableSelection"
            ...
            <el-button
              v-if="isAuth('shetuanhuodong','删除') && contents.btnAdAllIcon == 1 && contents.btnAdAllIconPosition == 2 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"
              type="danger"
              @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1?'删除':'' }}<i class="el-icon-delete el-icon--right" /></el-button>
            <el-button
              v-if="isAuth('shetuanhuodong','删除') && contents.btnAdAllIcon == 0 && contents.tableSelection"
              :disabled="dataListSelections.length <= 0"
              type="danger"
              @click="deleteHandler()"
            >{{ contents.btnAdAllFont == 1?'删除':'' }}</el-button>
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="活动地点">
		     <template slot-scope="scope">
                ...
                <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    prop="huodongshizhang"
                    header-align="center"
		    label="活动时长">
		     <template slot-scope="scope">
                       {{scope.row.huodongshizhang}}
                     </template>
                </el-table-column>
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                label="操作">
                <template slot-scope="scope">
                <el-button v-if="isAuth('shetuanhuodong','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                <el-button v-if="isAuth('shetuanhuodong','查看') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('shetuanhuodong','查看') && contents.tableBtnIcon == 0" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                <el-button v-if="isAuth('shetuanhuodong','报名参加') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="huodongcanyuCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'报名参加':'' }}</el-button>
                <el-button v-if="isAuth('shetuanhuodong','报名参加') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="huodongcanyuCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'报名参加':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('shetuanhuodong','报名参加') && contents.tableBtnIcon == 0" type="success" size="mini" @click="huodongcanyuCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'报名参加':'' }}</el-button>
    ...
    <huodongcanyu-cross-add-or-update v-if="huodongcanyuCrossAddOrUpdateFlag" :parent="this" ref="huodongcanyuCrossaddOrUpdate"></huodongcanyu-cross-add-or-update>
...
import huodongcanyuCrossAddOrUpdate from "../huodongcanyu/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      huodongcanyuCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    huodongcanyuCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.huodongcanyuCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','shetuanhuodong');
      this.$nextTick(() => {
      this.$refs.huodongcanyuCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
    },
